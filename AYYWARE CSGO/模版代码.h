...
template< typename 通用型 > 通用型 Call_从数组中取地址调用CALL(PVOID Base, DWORD Index)
{
	PDWORD* 基址 = (PDWORD*)Base; //基址 = 读内存(bae)
	PDWORD 数组数据 = *基址;//转换
	DWORD dwAddress = 数组数据[Index];//读内存(基址 + index *4)

	return (通用型)(dwAddress);
}



template< typename 通用型 > 通用型 Call_调用CALL(DWORD calladdress)
{
	return (通用型)(calladdress);
}
...
template< typename 通用型 > 通用型 call_fuck(PVOID Base, DWORD Index)
{
	char temp[64];
	int ss = (int)Base;
	sprintf(temp, "%d", ss);
...
#define Fuck(type) *(type*)dwThis
#define CNETVAR(type,offset) (type)(dwThis + offset)

//声明读内存子程序 - creates a netvar function in a class that returns value(s) [type=class type & return type] [name=function name] [netvar_crc=crc32 of netvar of choice]
#define 声明读内存子程序(type,name,netvar_crc) \
	type name() \
	{ \
		static DWORD dwObserverTarget=NetVar.GetNetVar(netvar_crc); \
		return NETVAR(type,dwObserverTarget); \
	}
#define 模版_读内存(type,name,idex) \
	type name() \
	{ \
		return NETVAR(type,idex); \
	}

#define 模版_取基址(type,name,idex) \
	type name() \
	{ \
		return Fuck(type); \
	}
...
//声明读内存子程序2 - creates a netvar function in a class that returns a pointer [type=class type & return type] [name=function name] [netvar_crc=crc32 of netvar of choice]
#define 声明读内存子程序2(type,name,netvar_crc) \
	type name() \
	{ \
		static DWORD dwObserverTarget=NetVar.GetNetVar(netvar_crc); \
